function generateBC(url, separador) {
    ...
    var migas = [];             // Array que contendrá las migas de pan ya formadas con las etiquetas <a> y <span>.
    ...
    // 3.1) Si la URL contiene más un componente, genero todas la migas excepto la última mediante un bucle for:
    if (componentesUrl.length > 1) {
        
        let ruta = "/"; // Variable que almacenará la ruta que tendrá el atributo href de cada miga.
    ...
    /* 3.2) Creo la última miga, o la única miga si la URL sólo contiene un único componente. Del mismo modo, 
...
function acronimo(componenteUrl, separador) {
    ...
    /* Extraigo las palabras dividiendo el componente por el separador y filtrando las palabras no permitidas 
